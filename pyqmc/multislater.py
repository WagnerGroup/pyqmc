...
def sherman_morrison_ms(e, inv, vec):
    ...
    invnew = ( 
        inv 
        - np.einsum("kdi,kdj->kdij", inv[:, :, :, e], tmp) / ratio[:, :, np.newaxis, np.newaxis]
    )   
...
class MultiSlater: 
    ...
    def recompute(self, configs):
        ...
        for s in [0,1]:
            ...
            self._dets.append(np.array(np.linalg.slogdet(mo_vals))) #Spin, (sign, val), nconf, [ndet_up, ndet_dn]
            self._inverse.append(np.linalg.inv(mo_vals))  #spin, Nconf, [ndet_up, ndet_dn], nelec, nelec
        return self.value()
    ...
    def updateinternals(self, e, epos, mask=None):
        ...
        det_ratio, self._inverse[s][mask,:,:,:] = sherman_morrison_ms(eeff, 
            self._inverse[s][mask,:,:,:], mo_vals[mask, :])
    ...
    def pgradient(self):
        ...
        det_coeff_grad = self._dets[0][0,:,self._det_map[0]]*self._dets[1][0,:,self._det_map[1]]*\
            np.exp(self._dets[0][1,:,self._det_map[0]] + self._dets[1][1,:,self._det_map[1]])
        
        curr_val = self.value()
        d["det_coeff"] = det_coeff_grad.T/(curr_val[0] * np.exp(curr_val[1]))[:,np.newaxis]
