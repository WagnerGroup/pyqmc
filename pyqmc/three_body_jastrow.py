...
class Three_Body_JastrowSpin:
    ...
    def recompute(self, configs):
        ...
        self.sum_ij[:, :, :, :, :, 0] = np.einsum(
            "inIk,jnIl,ijnm->nIklm", a_values[:nup], a_values[:nup], b_2d_values[0]
        )
        self.sum_ij[:, :, :, :, :, 1] = np.einsum(
            "inIk,jnIl,ijnm->nIklm", a_values[:nup], a_values[nup:], b_2d_values[1]
        )
        self.sum_ij[:, :, :, :, :, 2] = np.einsum(
            "inIk,jnIl,ijnm->nIklm", a_values[nup:], a_values[nup:], b_2d_values[2]
        )
    ...
    def updateinternals(self, e, epos, configs, mask=None, saved_values=None):
        ...
        self.val = np.log(self.testvalue(e,epos)[0])+self.val
    ...
    def testvalue(self, e, epos, mask=None):
        ...
        nconf, nelec = configs.configs.shape[:2]
        #edown is 1 if e is down spin and 0 for up spin
        edown = int(e >= self._mol.nelec[0])
        epos_old = configs.configs[:, e, :]
        rpos_old = np.linalg.norm(epos_old, axis=-1)

        nup = int(self._mol.nelec[0])
        ndown = int(self._mol.nelec[1])
        #sep keeps track of how many up electrons are left if you dont include e. nup-1 if e is up, else it is nup
        sep = nup - int(e < nup)
        #boolean array of size nelec. True for all indices except False for e.
        not_e = np.arange(self._nelec) != e

        #de_old = np.zeros((nconf, np.sum(self._mol.nelec), 3))
        de_old = configs.dist.dist_i(
            configs.configs[:, not_e, :], configs.configs[:, e, :]
        )
        re_old = np.linalg.norm(de_old, axis=-1)

        #de_new = np.zeros((nconf, np.sum(self._mol.nelec), 3))
        ...
        ae_old = a_values[e]
        a_values_not_e = a_values[not_e]
        a_values_up = a_values_not_e[:sep,...]
        a_values_down= a_values_not_e[sep:,...]

        b_values_old = np.zeros((self._nelec-1, nconf, len(self.b_basis)))
        b_values_new = np.zeros((self._nelec-1, nconf, len(self.b_basis)))

        for i, b in enumerate(self.b_basis):
            # swap axes: nconf and nelec. for now doing it here.
            b_values_old[:, :, i] = b.value(de_old, re_old).swapaxes(0,1)
            b_values_new[:, :, i] = b.value(de_new, re_new).swapaxes(0,1)
        ...
        e_partial_old = np.zeros((nconf,self._mol.natm,len(self.a_basis),len(self.a_basis),len(self.b_basis),3))
        e_partial_new = np.zeros((nconf,self._mol.natm,len(self.a_basis),len(self.a_basis),len(self.b_basis),3))
        
        #if e is down, then store in updown stream(index =1). if e is up store in upup stream(index=0).
        e_partial_old[...,edown] = np.einsum("nIk,jnIl,jnm->nIklm", ae_old, a_values_up, b_up_old)
        e_partial_new[...,edown] = np.einsum("nIk,jnIl,jnm->nIklm", ae_new, a_values_up, b_up_new)
        
        #if e is down, store in downdown stream (index=2), if e is up store in updown stream(index=1)
        e_partial_old[...,edown+1] =np.einsum("nIk,jnIl,jnm->nIklm", ae_old, a_values_down, b_down_old)
        e_partial_new[...,edown+1] =np.einsum("nIk,jnIl,jnm->nIklm", ae_new, a_values_down, b_down_new)

        delta_sum_ij = 2 * e_partial_new - 2 * e_partial_old
        value_diff = np.einsum('nIklms,Iklms->n',delta_sum_ij, self.parameters["ccoeff"])
        val = np.exp(value_diff)
        return val, None
