...
def compute_determinants(
    mo_coeff: jnp.ndarray,  # nbasis, norb
...
def compute_values_from_determinants(
    expansion: DeterminantExpansion,
    det_params: DeterminantParameters,
    dets_up: SlaterState,
    dets_down: SlaterState,
):
    logdets = jnp.take(dets_up.logabsdet, expansion.mapping_up) + jnp.take(
        dets_down.logabsdet, expansion.mapping_down
    )  # ndet
    signdets = jnp.take(dets_up.sign, expansion.mapping_up) * jnp.take(
        dets_down.sign, expansion.mapping_down
    )  # ndet
    ref = jnp.max(logdets)
...
compute_values_from_determinants = jax.vmap(
    compute_values_from_determinants, in_axes=(None, None, 0, 0), out_axes=(0, 0)
)


def evaluate_expansion(
    gto_evaluator,  # function (N,3) -> (N,nbasis)
    expansion: DeterminantExpansion,
    nelec: jnp.ndarray,  # (2,)
    det_params: DeterminantParameters,
    ...
    to create
    """
    dets_up = compute_determinants(
        det_params.mo_coeff_alpha,
        gto_evaluator,
        expansion.determinants_up,
        xyz[0 : nelec[0], :],
    )
    dets_down = compute_determinants(
        det_params.mo_coeff_beta,
        gto_evaluator,
...
vmap_lemma = jax.vmap(
    _determinant_lemma, in_axes=(None, 0, 0, None), out_axes=0
)  # over determinants


def testvalue_up(
    gto_evaluator,  # function (3) -> (nbasis)
    expansion: DeterminantExpansion,
    det_params: DeterminantParameters,
    dets_up: SlaterState,
    dets_down: SlaterState,
    ...
    ratios = vmap_lemma(
        mos, dets_up.inverse, expansion.determinants_up, e
    ).T  # deriv, ndet (the transpose for that last axis is important)
    newsigns = jnp.sign(ratios) * dets_up.sign
    newlogabs = dets_up.logabsdet + jnp.log(jnp.abs(ratios))
    logdets = jnp.take(newlogabs, expansion.mapping_up, axis=-1) + jnp.take(
        dets_down.logabsdet, expansion.mapping_down
    )  # ndet
    signdets = jnp.take(newsigns, expansion.mapping_up, axis=-1) * jnp.take(
        dets_down.sign, expansion.mapping_down
    )  # ndet

    logdets_old = jnp.take(dets_up.logabsdet, expansion.mapping_up) + jnp.take(
        dets_down.logabsdet, expansion.mapping_down
    )  # ndet
    signdets_old = jnp.take(dets_up.sign, expansion.mapping_up) * jnp.take(
        dets_down.sign, expansion.mapping_down
    )  # ndet

    ref = jnp.max(jnp.concat((logdets.flatten(), logdets_old)))
    values = jnp.sum(
        det_params.ci_coeff * signdets * jnp.exp(logdets - ref), axis=-1
    )  # derivatives
    values_old = jnp.sum(
        det_params.ci_coeff * signdets_old * jnp.exp(logdets_old - ref), axis=-1
    )  # scalar
...
def testvalue_down(
    ...
    ratios = vmap_lemma(
        mos, dets_down.inverse, expansion.determinants_down, e
    ).T  # ndet ratios

    newsigns = jnp.sign(ratios) * dets_down.sign
    newlogabs = dets_down.logabsdet + jnp.log(jnp.abs(ratios))
    logdets = jnp.take(dets_up.logabsdet, expansion.mapping_up) + jnp.take(
        newlogabs, expansion.mapping_down, axis=-1
    )  # ndet
    signdets = jnp.take(dets_up.sign, expansion.mapping_up) * jnp.take(
        newsigns, expansion.mapping_down, axis=-1
    )  # ndet
    ...
    values_old = jnp.sum(
        det_params.ci_coeff * signdets_old * jnp.exp(logdets_old - ref)
    )  # scalar
    ratio = values / values_old
    return ratio, mos
...
def create_wf_evaluator(mol, mf, det_tol=1e-9, nimages=2, mc=None):
    ...
    ci_coeff, determinants, mapping = pyqmc.wf.determinant_tools.create_packed_objects(
        _determinants, tol=det_tol
    )
    ...
    expansion = DeterminantExpansion(
        mapping[0], mapping[1], jnp.array(determinants[0]), jnp.array(determinants[1])
    )
    ...
    _testval_up_batch = jax.vmap(
        _testvalue_up, in_axes=(None, None, None, None, 0), out_axes=0
    )
    _testval_down_batch = jax.vmap(
        _testvalue_down, in_axes=(None, None, None, None, 0), out_axes=0
    )
    ...
    value_func = [
        jax.jit(
            jax.vmap(
                ...
                out_axes=(0, 0, SlaterState(0, 0, 0, 0), SlaterState(0, 0, 0, 0)),
            )
        )
        for f in value_func
    ]
...
class _parameterMap:
    ...
    def __init__(self, jax_parameters):
        ...
        self.parameters = {
            "det_coeff": np.asarray(self.jax_parameters[0]),
    ...
    def __setitem__(self, key, item):
        ...
        self.jax_parameters = DeterminantParameters(
            jnp.array(self.parameters["det_coeff"]),
...
class JAXSlater:
    def __init__(self, mol, mf, nimages=2, tol=1e-9, mc=None):
        (
            _parameters,
            self.expansion,
            (self._recompute, self._pgradient),
            (
                _testvalue_up,
                _testvalue_down,
                _grad_up,
    ...
    def recompute(self, configs):
        ...
        self._sign, self._logabs, self._dets_up, self._dets_down = self._recompute(
            self.parameters.jax_parameters, xyz
        )
    ...
    def updateinternals(self, e, epos, configs, mask=None, saved_values=None):
        ...
        if spin == 0:
            ratio, inverse = sherman_morrison_row(
                e, self._dets_up.inverse, saved_values, self.expansion.determinants_up
            )
            newsigns = jnp.sign(ratio) * self._dets_up.sign
            newlogabs = self._dets_up.logabsdet + jnp.log(jnp.abs(ratio))
            inverse = jnp.where(
                mask[:, jnp.newaxis, jnp.newaxis, jnp.newaxis],
                inverse,
                self._dets_up.inverse,
            )

            newsigns = jnp.where(mask[:, jnp.newaxis], newsigns, self._dets_up.sign)
            newlogabs = jnp.where(
                mask[:, jnp.newaxis], newlogabs, self._dets_up.logabsdet
            )
            ...
            ratio, inverse = sherman_morrison_row(
                e,
                self._dets_down.inverse,
                saved_values,
                self.expansion.determinants_down,
            )
            ...
            newsigns = jnp.where(
                mask[:, jnp.newaxis], newsigns, self._dets_down.sign
            )  # (nconf, ndet)
            newlogabs = jnp.where(
                mask[:, jnp.newaxis], newlogabs, self._dets_down.logabsdet
            )
            self._dets_down = SlaterState(None, newsigns, newlogabs, inverse)

        self._sign, self._logabs = compute_values_from_determinants(
            self.expansion,
    ...
    def testvalue(self, e, epos, mask=None):
        ...
        e = e - self._nelec[0] * spin
        if (
            len(xyz.shape) == 3 and mask is not None
        ):  # This is an optimization for ECPs.
            if True:
                nmask = np.sum(mask)
                ...
                xyz = jnp.pad(
                    xyz[mask],
                    ((0, self._cached_pad_ecp - nmask), (0, 0), (0, 0)),
                    mode="empty",
                )
                # print(xyz.shape)
                det_up = SlaterState(
                    None,
                    jnp.pad(
                        self._dets_up.sign[mask],
                        ((0, self._cached_pad_ecp - nmask), (0, 0)),
                        mode="constant",
                    ),
                    jnp.pad(
                        self._dets_up.logabsdet[mask],
                        ((0, self._cached_pad_ecp - nmask), (0, 0)),
                        mode="constant",
                    ),
                    jnp.pad(
                        self._dets_up.inverse[mask],
                        ((0, self._cached_pad_ecp - nmask), (0, 0), (0, 0), (0, 0)),
                        mode="constant",
                    ),
                )
                det_down = SlaterState(
                    None,
                    jnp.pad(
                        self._dets_down.sign[mask],
                        ((0, self._cached_pad_ecp - nmask), (0, 0)),
                        mode="constant",
                    ),
                    jnp.pad(
                        self._dets_down.logabsdet[mask],
                        ((0, self._cached_pad_ecp - nmask), (0, 0)),
                        mode="constant",
                    ),
                    jnp.pad(
                        self._dets_down.inverse[mask],
                        ((0, self._cached_pad_ecp - nmask), (0, 0), (0, 0), (0, 0)),
                        mode="constant",
                    ),
                )
                newvals, saved = self._testvalue_batch[spin](
                    self.parameters.jax_parameters, det_up, det_down, e, xyz
                )
            ...
            len(xyz.shape) == 3
        ):  # for a batch of configurations where there is no mask
            newvals, saved = self._testvalue_batch[spin](
                self.parameters.jax_parameters, self._dets_up, self._dets_down, e, xyz
            )
            return newvals, saved
        else:
            newvals, saved = self._testvalue[spin](
                self.parameters.jax_parameters, self._dets_up, self._dets_down, e, xyz
            )
            if mask is not None:
                return np.array(newvals)[mask], saved
    ...
    def gradient(self, e, epos, mask=None):
        ...
        spin = int(e >= self._nelec[0])
        e = e - self._nelec[0] * spin
        grad, saved = self._grad[spin](
            self.parameters.jax_parameters, self._dets_up, self._dets_down, e, xyz
        )  # pyqmc wants (3, nconfig)
    ...
    def gradient_value(self, e, epos):
        ...
        spin = int(e >= self._nelec[0])
        e = e - self._nelec[0] * spin
        derivatives, saved = self._grad[spin](
            self.parameters.jax_parameters, self._dets_up, self._dets_down, e, xyz
        )  # pyqmc wants (3, nconfig)
        # print("saved", saved.shape)
        convert = (
            derivatives[:, 1:].T / derivatives[:, 0],
            derivatives[:, 0],
            saved[:, 0, :],
        )
        return convert
    ...
    def gradient_laplacian(self, e, epos):
        ...
        spin = int(e >= self._nelec[0])
        e = e - self._nelec[0] * spin
        laplacian, saved = self._lap[spin](
            self.parameters.jax_parameters, self._dets_up, self._dets_down, e, xyz
        )
        return np.array(laplacian[:, 1:4].T / laplacian[:, 0]), np.asarray(
            laplacian[:, 4] / laplacian[:, 0]
        )
    ...
    def pgradient(self):
        ...
        grads = self._pgradient(
            self.parameters.jax_parameters, xyz
        )  # [1] # sign, log, dets_up, dets_down
        return {
            "det_coeff": np.array(grads[0]),
