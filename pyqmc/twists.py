...
def create_supercell_twists(supercell, mf, tol=4):
    """

    Args: 
    supercell: a supercell object made from pyqmc.supercell.make_supercell
    mf: a mf object with KRKS
    tol: number of decimals to round the k-points. Default is 1/1000 which should be ok for most meshes
    ...
    twists, indices, counts = np.unique(np.round(super_kpts,tol), axis=0, return_counts=True, return_inverse=True)
    kinds = []
