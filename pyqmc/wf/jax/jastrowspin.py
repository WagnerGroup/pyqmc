...
def partial_basis_sum_grad(
    elec_coord, coords, coeff, basis_grad, param, rcut, wrap_vecs
):
    """
...
def vmap_basis(basis):
    ...
    return jax.vmap(
        jax.vmap(
            basis, in_axes=(0, None, None), out_axes=0
        ),  # [(n,), float, float] -> (n,)
        in_axes=(None, 0, None),
        out_axes=1,
...
def evaluate_jastrow(mol, basis_params, partial_sum_evaluators, parameters, coords):
    ...
    a_up = a_eval(coords_up, atom_coords, acoeff[:, 1:, 0]) + cusp_eval(
        coords_up, atom_coords, acoeff[:, :1, 0]
    )  # (nelec,)
    a_dn = a_eval(coords_dn, atom_coords, acoeff[:, 1:, 1]) + cusp_eval(
        coords_dn, atom_coords, acoeff[:, :1, 1]
    )

    # compute b terms for up-up, up-down, down-down electron pairs
    # bcoeff is tiled to match the first dimension (n) of coords_spin
    b_upup = b_eval(
        coords_up, coords_up, jnp.tile(bcoeff[1:, 0], (nup, 1))
    ) + cusp_eval(coords_up, coords_up, jnp.tile(bcoeff[:1, 0], (nup, 1)))  # (nelec,)
    b_updn = b_eval(
        coords_up, coords_dn, jnp.tile(bcoeff[1:, 1], (ndn, 1))
    ) + cusp_eval(coords_up, coords_dn, jnp.tile(bcoeff[:1, 1], (ndn, 1)))
    b_dndn = b_eval(
        coords_dn, coords_dn, jnp.tile(bcoeff[1:, 2], (ndn, 1))
...
def evaluate_testvalue(
    mol, partial_sum_evaluators, parameters, coords_old, e, spin, epos
):
    """
    ...
    a = a_eval(epos, atom_coords, acoeff[:, 1:, spin]) + cusp_eval(
        epos, atom_coords, acoeff[:, :1, spin]
    )  # (float)
    a_old = a_eval(epos_old, atom_coords, acoeff[:, 1:, spin]) + cusp_eval(
        epos_old, atom_coords, acoeff[:, :1, spin]
    )

    # compute b terms contributed by electron e
    # bcoeff is tiled to match the first dimension (n) of coords_spin
    b_up = b_eval(
        epos, coords_up, jnp.tile(bcoeff[1:, spin + 0], (nup, 1))
    ) + cusp_eval(epos, coords_up, jnp.tile(bcoeff[:1, spin + 0], (nup, 1)))  # (float)
    b_dn = b_eval(
        epos, coords_dn, jnp.tile(bcoeff[1:, spin + 1], (ndn, 1))
    ) + cusp_eval(epos, coords_dn, jnp.tile(bcoeff[:1, spin + 1], (ndn, 1)))
    b_up_old = b_eval(
        epos_old, coords_old_up, jnp.tile(bcoeff[1:, spin + 0], (nup, 1))
    ) + cusp_eval(epos_old, coords_old_up, jnp.tile(bcoeff[:1, spin + 0], (nup, 1)))
    b_dn_old = b_eval(
        epos_old, coords_old_dn, jnp.tile(bcoeff[1:, spin + 1], (ndn, 1))
...
def evaluate_derivative(
    mol, partial_sum_derv_evaluators, parameters, coords_up, coords_dn, spin, epos
):
    """
    ...
    a_derv = a_derv_eval(epos, atom_coords, acoeff[:, 1:, spin]) + cusp_derv_eval(
        epos, atom_coords, acoeff[:, :1, spin]
    )  # (3,) or (3, 3)

    # compute derivative of b terms
    # bcoeff is tiled to match the first dimension (n) of coords_spin
    b_derv_up = b_derv_eval(
        epos, coords_up, jnp.tile(bcoeff[1:, spin + 0], (nup, 1))
    ) + cusp_derv_eval(
        epos, coords_up, jnp.tile(bcoeff[:1, spin + 0], (nup, 1))
    )  # (3,) or (3, 3)
    b_derv_dn = b_derv_eval(
        epos, coords_dn, jnp.tile(bcoeff[1:, spin + 1], (ndn, 1))
...
def evaluate_pgradient(
    mol, basis_params, partial_sum_pgrad_evaluators, parameters, coords
):
    """
    ...
    a_up_pgrad = a_pgrad_eval(
        coords_up, atom_coords, acoeff[:, 1:, 0]
    )  # (nelec, natom, nbasis, 2)
    ...
    b_upup_pgrad = (
        b_pgrad_eval(coords_up, coords_up, jnp.tile(bcoeff[1:, 0], (nup, 1))) / 2
    )  # (nelec, nelec, nbasis, 3)
    bc_upup_pgrad = (
        cusp_pgrad_eval(coords_up, coords_up, jnp.tile(bcoeff[:1, 0], (nup, 1))) / 2
    )
    b_updn_pgrad = b_pgrad_eval(coords_up, coords_dn, jnp.tile(bcoeff[1:, 1], (ndn, 1)))
    bc_updn_pgrad = cusp_pgrad_eval(
        coords_up, coords_dn, jnp.tile(bcoeff[:1, 1], (ndn, 1))
    )
    b_dndn_pgrad = (
        b_pgrad_eval(coords_dn, coords_dn, jnp.tile(bcoeff[1:, 2], (ndn, 1))) / 2
    )
    bc_dndn_pgrad = (
        cusp_pgrad_eval(coords_dn, coords_dn, jnp.tile(bcoeff[:1, 2], (ndn, 1))) / 2
    )
    rcut, gamma = basis_params.rcut, basis_params.gamma[0]
    ...
    b_pgrad = b_pgrad.at[:1, 0].set(
        jnp.sum(bc_upup_pgrad, axis=(0, 1)) - nup * diagc / 2
    )
    ...
    b_pgrad = b_pgrad.at[:1, 2].set(
        jnp.sum(bc_dndn_pgrad, axis=(0, 1)) - ndn * diagc / 2
    )
...
def create_jastrow_evaluator(mol, basis_params):
    ...
    if hasattr(mol, "lattice_vectors"):
        wrap_vecs = lambda rij_vecs: minimum_image(
            rij_vecs, jnp.array(mol.lattice_vectors())
        )
    else:
    ...
    psum_evaluators = [
        partial(
            partial_basis_sum, basis=basis, param=param, rcut=rcut, wrap_vecs=wrap_vecs
        )
        for basis, param in zip(basis_vm_lst, basis_params_lst)
    ]
    psum_grad_evaluators = [
        partial(
            partial_basis_sum_grad,
            basis_grad=basis,
            param=param,
            rcut=rcut,
            wrap_vecs=wrap_vecs,
    ...
    psum_hess_evaluators = [
        partial(
            jax.hessian(partial_basis_sum, argnums=0),
            basis=basis,
            param=param,
            rcut=rcut,
            wrap_vecs=wrap_vecs,
    ...
    psum_pgrad_evaluators = [
        jax.vmap(  # vmapping over electrons
            partial(
                jax.grad(partial_basis_sum, argnums=2),
                basis=basis,
                param=param,
                rcut=rcut,
                wrap_vecs=wrap_vecs,
...
class _parameterMap:
    ...
    def __init__(self, jax_parameters, has_ion_cusp):
        ...
        if self.has_ion_cusp:
            self.parameters = {
                "acoeff": np.asarray(self.jax_parameters.acoeff),
                "bcoeff": np.asarray(self.jax_parameters.bcoeff),
            }
        else:
            self.parameters = {
                "acoeff": np.asarray(self.jax_parameters.acoeff[:, 1:, :]),
                "bcoeff": np.asarray(self.jax_parameters.bcoeff),
            }
    ...
    def __setitem__(self, key, item):
        ...
        if self.has_ion_cusp:
            self.jax_parameters = CoefficientParameters(
                jnp.array(self.parameters["acoeff"]),
                jnp.array(self.parameters["bcoeff"]),
            )
        else:
            padded_acoeff = np.pad(self.parameters["acoeff"], ((0, 0), (1, 0), (0, 0)))
            self.jax_parameters = CoefficientParameters(
                jnp.array(padded_acoeff), jnp.array(self.parameters["bcoeff"])
            )
...
class JAXJastrowSpin:
    def __init__(
        ...
        (
            ...
            self._hessian,
            self._pgradient,
        ) = create_jastrow_evaluator(self._mol, self.basis_params)
    ...
    def _init_params(self, ion_cusp, na, nb, rcut, gamma, beta0_a, beta0_b):
        ...
        if rcut is None:
            if hasattr(mol, "a"):
                rcut = jnp.amin(
                    jnp.pi / jnp.linalg.norm(mol.reciprocal_vectors(), axis=1)
                )
            else:
                rcut = 7.5
        ...
        self.parameters = _parameterMap(
            CoefficientParameters(acoeff, bcoeff), self.has_ion_cusp
        )
    ...
    def testvalue(self, e, epos, mask=None):
        ...
        testval = evaluator(
            self.parameters.jax_parameters, _configs_old, e, spin, _epos
        )[jnpmask]
    ...
    def gradient(self, e, epos):
        ...
        return self._gradient(
            self.parameters.jax_parameters, configs_up, configs_dn, spin, _epos
        ).T
