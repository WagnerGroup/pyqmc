...
def gradient_lines(wf, configs, pgrad, warmup=10,
        iters=10, vmc=None,vmcoptions=None,
    ...
    for i in range(iters): 
        params, fitval, (val,grad) = line_minimization(grad_energy_function, params, **vmcoptions)
        data['iter'].append(i)
        ...
        if verbose>0: 
            print('iteration %i'%i, 'fitval', fitval, 'val', val, '|grad|', np.linalg.norm(grad), 'time',time.process_time()-start)
            print('='*30)
...
def test():
    ...
    gradient_lines(wf,coords,pgrad_acc,warmup=10, iters=5,vmc=distvmc,verbose=1,
