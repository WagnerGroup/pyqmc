...
class TBDMAccumulator:
    ...
    Args:
      ...
      spin (uple): size 2 indicates spin sector to be computed, either (0,0), (0,1), (1,0) or (1,1)
    ...
    def __init__(
        ...
        electrons_b = np.arange(spin[1]*mol.nelec[0], mol.nelec[0]+spin[1]*mol.nelec[1])
        ...
        self._aux_configs_a = initial_guess(mol, int(naux/sum(self._mol.nelec))).configs.reshape(-1, 3)
        ...
        self._aux_configs_b = initial_guess(mol, int(naux/sum(self._mol.nelec))).configs.reshape(-1, 3)
        for i in range(warmup):
    ...
    def __call__(self, configs, wf):
        ...
        results["value"] = np.zeros((nconf, orb_a_size, orb_a_size, orb_b_size, orb_b_size))
        for i,e in enumerate(['a','b']):
            results["norm_%s"%e] = np.zeros((nconf, self._orb_coeff[self._spin_sector[i]].shape[1]))
        ...
        for sweep in range(self._nsweeps):
            for i,pair in enumerate(self._pairs):                
                ...
                points_a = np.concatenate([aux_configs_a[i*naux_a:(i+1)*naux_a], configs.configs[:, pair[0], :]])
                points_b = np.concatenate([aux_configs_b[i*naux_b:(i+1)*naux_b], configs.configs[:, pair[1], :]])
                ...
                epos_a = configs.make_irreducible(
                    pair[0], aux_configs_a[i*naux_a:(i+1)*naux_a][auxassignments_a[i+sweep*len(self._pairs)]]
                )
                epos_a_orig = configs.electron(pair[0])
                epos_b = configs.make_irreducible(
                    pair[1], aux_configs_b[i*naux_b:(i+1)*naux_b][auxassignments_b[i+sweep*len(self._pairs)]]
                )
                ...
                orbratio = np.einsum(
                    "mj,ml,mi,mk->mijkl",
                    orb_a_aux[auxassignments_a[i+sweep*len(self._pairs)], :] / fsum_a[auxassignments_a[i+sweep*len(self._pairs)], np.newaxis],
                    orb_b_aux[auxassignments_b[i+sweep*len(self._pairs)], :] / fsum_b[auxassignments_b[i+sweep*len(self._pairs)], np.newaxis],
                    orb_a_configs, orb_b_configs,
                )

                # Adding to results
                results["value"] += np.einsum("i,ijklm->ijklm", wfratio, orbratio)
                results["norm_a"] += norm_a[auxassignments_a[i+sweep*len(self._pairs)]]
                results["norm_b"] += norm_b[auxassignments_b[i+sweep*len(self._pairs)]]
